...
<div class="card-body">
    ...
    <div class="table-responsive">
              ...
              <table id="dtBasicExample" class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">  نوعیت اسناد </th>
                                <th scope="col">  تاریخ ثبت     </th>
                                <th scope="col">    شماره مسلسل   </th>                                   
                            </tr>
                        </thead>
                        <tbody class="text-right">
                            @foreach($ahkam as $file)
                                    <?php 
                                ...
                                <tr>
                                    <td>
                                        <div class="btn-group ml-auto" style="direction:rtl;">
                                            <a href="{{url('panel_ahkam/view',['file' => $file->id])}}" title="نمایش سند"  class="btn btn-sm btn-outline-light text-dark"> <i class="far fas fa-eye"></i></a>   
                                            @if(!(Auth::user()->type === 3))    
                                            ...
                                            <a href="{{url('panel_ahkam/access',['file' => $file->id])}}" title="دسترسی دادن کاربران" class="btn btn-sm btn-outline-light text-dark" > <i class="far fas fa-user"></i> </a>
                                            <a href="{{url('panel_ahkam/update',['file' => $file->id])}}" title="ویرایش سند" class="btn btn-sm btn-outline-light text-dark" > <i class="far fas fa-edit"></i></a>
                                            @endif
                                            @if(Auth::user()->type === 1)
                                    ...
                                    <td class="td-control">{{$file->type_of_document}}</td>
      ...
      <span id="base_url" hidden>{{asset('/')}}</span>
        ...
        <div class="modal fade" id="ahkam_users" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
$(document).ready(function () {
    ...
    $(".delete").click(function(){
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.get(base_url+"panel_ahkam/delete/"+id, function(data){
