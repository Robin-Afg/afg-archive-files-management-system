...
<div class="card-body">
    ...
    <div class="table-responsive">
                                ...
                                <tr>
                                    <td>
                                        <div class="btn-group ml-auto" style="direction:rtl;">
                                            <a href="{{url('panel_saderamali/view',['file' => $file->id])}}" title="نمایش سند"  class="btn btn-sm btn-outline-light text-dark"> <i class="far fas fa-eye"></i></a>   
                                            @if(!(Auth::user()->type === 3))   
                                            ...
                                            <a href="{{url('panel_saderamali/access',['file' => $file->id])}}" title="دسترسی دادن کاربران" class="btn btn-sm btn-outline-light text-dark" > <i class="far fas fa-user"></i> </a>
                                            <a href="{{url('panel_saderamali/update',['file' => $file->id])}}" title="ویرایش سند" class="btn btn-sm btn-outline-light text-dark" > <i class="far fas fa-edit"></i></a>
                                            @endif
                                            @if(Auth::user()->type === 1)
      ...
      <span id="base_url" hidden>{{asset('/')}}</span>
        ...
        <div class="modal fade" id="saderamali_users" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
$(document).ready(function () {
    ...
    $(".view-users").click(function () {
      var id = $(this).attr('data-id');
       $.get(base_url+"get_data/panel_saderamali/"+id, function(data){
        var string = "";
        data = data[0].authorized_users;
        for (var i = 0; i < data.length ; i++) {
           string += ' <span class="badge badge-pill badge-info fa-1x">'+data[i].user.name+'</span>';
        }
               $("#saderamali_users .modal-body").html(string);
               $("#saderamali_users").modal();
            });
    });
...
$(document).ready(function () {
    ...
    $(".delete").click(function(){
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.get(base_url+"panel_saderamali/delete/"+id, function(data){
